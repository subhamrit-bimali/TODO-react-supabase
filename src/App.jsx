...
export default function App() {
  ...
  const handleAdd = async (e) => {
    ...
    if (error) setError(error.message)
    else { setNewTitle(''); setPriority('none') }
    setAdding(false)
  }

  const handleToggle = async (id, currentState) => {
    const { error } = await supabase
      .from('todos').update({ is_complete: !currentState }).eq('id', id)
    if (error) setError(error.message)
  }
        ...
        <div className={styles.container}>
          ...
          <form className={styles.addForm} onSubmit={handleAdd}>
            <div className={styles.addRow}>
              ...
              >
                {adding ? <span className={styles.spinner} /> : (
            ...
            <div className={styles.filters}>
              {FILTERS.map(f => (
                ...
                >
                  ...
                  {f === 'active' && activeCount > 0 && <span className={styles.badge}>{activeCount}</span>}
                  {f === 'done' && doneCount > 0 && <span className={styles.badgeDone}>{doneCount}</span>}
                </button>
              ))}
            </div>
            {doneCount > 0 && (
          ...
          <div className={styles.list}>
            {loading ? (
              ...
              <div className={styles.empty}>
                {filter === 'done'
                  ? <><span className={styles.emptyIcon}>✓</span><p>No completed tasks yet</p></>
                  : filter === 'active'
                  ? <><span className={styles.emptyIcon}>🎉</span><p>All caught up!</p></>
                  : <><span className={styles.emptyIcon}>○</span><p>No tasks yet — add one above</p></>
                }
              </div>
            ) : (
              filtered.map(todo => (
